        ...
        {sent ? (
            ...
            !submitted ? (
                <div className='m-4'>
                    ...
                    <form className='max-w-[720px] p-8 rounded-2xl bg-white [box-shadow:inset_4px_4px_4px_rgba(0,0,0,25%)]' onSubmit={(e: React.FormEvent<HTMLFormElement>) => handleSubmit(e)}>
                        <TextField className='mt-4' fullWidth label='お名前' value={name} type='name' onChange={e => setName(e.target.value)} error={nameError} helperText={nameError ? '名前を入力してください' : ''} required />
                        <TextField className='m-[2rem_auto]' fullWidth label='メールアドレス' value={email} type='email' onChange={e => setEmail(e.target.value)} error={emailError} helperText={emailError ? 'メールアドレスを入力してください' : ''} required />
                        <TextField className='mb-[2rem]' fullWidth label='件名' value={subject} type='subject' onChange={e => setSubject(e.target.value)} />
                        <TextField fullWidth label='お問い合わせ内容' multiline rows={4} value={message} type='textarea' onChange={e => setMessage(e.target.value)} error={messageError} helperText={messageError ? '内容を入力してください' : ''} required />
                        <div className='w-full flex justify-center'>
                            <Button variant='contained' color='primary' type='submit' className='mt-[2rem] bg-[#1976d2]'>入力内容の確認へ</Button>
                        </div>
                    </form>
                </div>
            ) : (
                <div className="confirmation w-[80%] max-w-[720px] p-8 rounded-2xl bg-white [box-shadow:inset_4px_4px_4px_rgba(0,0,0,25%)]">
                    ...
                    <form onSubmit={(e: React.FormEvent<HTMLFormElement>) => handleSend(e)}>
                        ...
                        <div className='flex justify-center [border-top:1px_solid_#ddd]'>
                            <Button variant='contained' color='error' type='button' onClick={() => setSubmitted(false)} className='bg-[#d32f2f] mt-5'>戻る</Button>
                            <Button variant='contained' color='primary' type='submit' className='bg-[#1976d2] ml-4 mt-5'>送信</Button>
                        </div>
                    </form>
                </div>
            )
